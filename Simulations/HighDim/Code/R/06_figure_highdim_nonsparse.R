...
# 03: organize results and make figures, truth is nonsparse
###################################################################
rm(list = ls())
...
load("../../Results/05_highdim_nonsparse.RData")
...
p1 = ggplot(S_cor) + geom_boxplot(aes(x=IC, y=cor,fill = Method)) + facet_grid(~Method)+ 
...
p2 = ggplot(M_cor) + geom_boxplot(aes(x=IC, y=cor,fill = Method)) + facet_grid(~Method) + 
...
ggsave("../../Figures/06_highdim_nonsparse_single.pdf",width = 14, height = 6)
...
ggarrange(p1, p2, 
          ...
          ncol = 2, nrow = 1,
          common.legend = T,legend = "bottom")
ggsave("../../Figures/06_highdim_nonsparse_single_nomatlab.pdf",width = 14, height = 6)


###################################################################
# Results for constructing the table
###################################################################
