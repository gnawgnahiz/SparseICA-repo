...
# 05: run the simulations in setting 1, non-sparse truth
###################################################################
...
load("../../Data/sim123_nonsparse_lowSNR.RData")
for (i in 1:rep) {
  xmat = xmat_list[[i]]
  smat = smat_list[[i]]
  mmat = mmat_list[[i]]
  ...
  ###############################################################################
  filename = paste0("../../Results/nonsparse/EBM/low/estS_",i,".mat")
  ...
  ###############################################################################
  filename = paste0("../../Results/nonsparse/sparsefast/low/estS_",i,".mat")
  dat = readMat(filename)
